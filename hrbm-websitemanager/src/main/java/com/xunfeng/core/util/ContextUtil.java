...
* @Description: 取得当前用户登录时的上下文环境，一般用于获取当前登录的用户
* @author wanghan 
* @createDate 2015年4月23日-上午11:44:37 
* @version V1.0   
*/
public class ContextUtil {
	...
	private static ThreadLocal<Locale> curLocale = new ThreadLocal<Locale>();
 	...
 	private static ThreadLocal<SysDept> curDept = new ThreadLocal<SysDept>();
 	private static ThreadLocal<SysConfig> curDistrict = new ThreadLocal<SysConfig>();
...
//	private static ThreadLocal<Position> curPos=new ThreadLocal<Position>();
	...
	public static SysConfig getLoginTitle(){
		 
		return getSysConfig("login_title");
		 
	}
	public static SysConfig getIndexTitle(){
		 
		return getSysConfig("index_title");
	}
	
	public static SysConfig getSysConfig(String name){
		 
		SysConfigServiceInter sysConfig = (SysConfigServiceInter)AppUtil.getBean("config");
		return sysConfig.getConfigByName(name);
	 ...
	 * 获取当前用户所在的部门
	 */
	public static SysDept getCurrentUserDept(){
		if(curDept.get()!=null){
			return curDept.get();
		}
		SysUser sysUser = getCurrentUser();
		Long deptId = sysUser.getDep001();
		if(null!=deptId){
			SysDeptServiceInterf sysDeptService = 	(SysDeptServiceInterf)AppUtil.getBean("sysdept");
	 ...
	 * 获取当前用户所在的组织
	 */
	public static SysOrg getCurrentUserOrg(){
		if(curOrg.get()!=null){
			return curOrg.get();
		}
		SysUser sysUser = getCurrentUser();
...
//	}
	...
	public static Long getCurrentUserId(){
		SysUser curUser=getCurrentUser();
		if(curUser!=null) return curUser.getAcd230();
		return 0L;
	}
	/**
	 * 设置当前用户账号
...
//		SysUserService sysUserService=(SysUserService)AppUtil.getBean("sysuser");
