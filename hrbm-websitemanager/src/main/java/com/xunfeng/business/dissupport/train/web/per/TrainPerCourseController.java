...
*/
@Controller
@RequestMapping("/business/dissupport/train/trainCourse/per")
public class TrainPerCourseController extends BaseController
{
	@Resource
	...
	public void save(HttpServletRequest request, HttpServletResponse response,TrainCourseForm trainCourseForm) throws Exception
	{
		...
		try{
			...
			if(trainCourseForm.isMvcAdd()){
				...
				TrainOrg to=trainOrgService.getById(Long.valueOf(orgId));
				Date date=new Date();//时间
				trainCourse.setAae019(to==null? "" : to.getAae019());// 经办人
				trainCourse.setAae020(to==null? "" : to.getAae020());// 经办机构
				trainCourse.setAae011(to==null? "" : to.getAae011());// 经办人代码
				trainCourse.setAae017(to==null? "" : to.getAae017());// 经办机构代码
				trainCourse.setIsDel("0");//默认未被删除
				if("0".equals(flag)){
					trainCourse.setIsAudit("1");//审核通过
					trainCourse.setAbb767(to==null? "" : to.getAae011());// 审核人代码
					trainCourse.setAbb768(to==null? "" : to.getAae017());// 审核机构代码
					trainCourse.setAbb769(to==null? "" : to.getAae019());// 审核人
					trainCourse.setAbb770(to==null? "" : to.getAae020());// 审核机构
					trainCourse.setAbb771(date);// 审核时间
				}else{
					trainCourse.setIsAudit("0");//默认为0:未审核
				}
				trainCourse.setIsAudit("0");//默认未审核
	...
	{	
		ModelAndView mv=new ModelAndView("/business/dissupport/train/per/trainCoursePerList.jsp");
		String orgId=RequestUtil.getString(request, "orgId");
	...
	{	
	    ...
	    fiter.addFilter("isDel", "0");//未被删除的
	    fiter.addFilter("pxOrgIds", orgId);
	...
	{   
		...
		ModelAndView mv= new ModelAndView("/business/dissupport/train/per/trainCoursePerAdd.jsp");
		mv.addObject("orgId", orgId);
		QueryFilter fiter=new QueryFilter(request);
		fiter.addFilter("isDel", "0"); //未被删除的
		List<TrainCourseType> courseTypeList=trainCourseTypeService.getAll(fiter);
		//获取培训机构
		List<TrainOrg> orgList=new ArrayList<TrainOrg>();
		orgList.add(trainOrgService.getById(Long.valueOf(orgId)));
		return mv.addObject("trainCourseForm",trainCourseForm).addObject("courseTypeList", courseTypeList).addObject("orgList", orgList);					
	}
	/**
	 * 	编辑开班信息表
	 * @param request
	...
	public ModelAndView edit(HttpServletRequest request) throws Exception
	{
		...
		String orgId=RequestUtil.getString(request, "orgId");
		ModelAndView mv= new ModelAndView("/business/dissupport/train/per/trainCoursePerEdit.jsp");
		mv.addObject("orgId", orgId);
		TrainCourse trainCourse=trainCourseService.getById(pxCourseId);
		TrainCourseForm  trainCourseForm=new TrainCourseForm();
		...
		List<TrainOrg> orgList=new ArrayList<TrainOrg>();
		orgList.add(trainOrgService.getById(Long.valueOf(orgId)));
		return mv.addObject("trainCourseForm",trainCourseForm).addObject("courseTypeList", courseTypeList)
	...
	public ResultMessage update(HttpServletRequest request, HttpServletResponse response) throws Exception
	{
		...
		try{
			...
			if(flag.equals("audit")){
				...
				paramMap.put("abb769",ContextUtil.getCurrentUser().getAac003());//审核人
				paramMap.put("abb767",ContextUtil.getCurrentUser().getAcd230()+"");//审核人代码
				paramMap.put("abb770",ContextUtil.getCurrentUserOrg().getAcd202());//审核机构
				paramMap.put("abb768",ContextUtil.getCurrentUserOrg().getAcd200()+"");//审核机构代码
				paramMap.put("abb772",abb772);//审核意见
				paramMap.put("abb771",new Date());//审核时间
			}else{
				TrainOrg to=trainOrgService.getById(Long.valueOf(orgId));
				paramMap.put("aae019",to==null? "" : to.getAae019());//经办人
				paramMap.put("aae011",to==null? "" : to.getAae011());//经办人代码
				paramMap.put("aae020",to==null? "" : to.getAae020());//经办机构
				paramMap.put("aae017",to==null? "" : to.getAae017());//经办机构代码
