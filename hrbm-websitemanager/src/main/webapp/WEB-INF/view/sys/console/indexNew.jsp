...
<html lang="en">
	<head>
		...
		-->
		<!-- ace styles -->
		<link rel="stylesheet" href="${ctx}/assets/css/ace.min.css?version=20161205" />
    	...
    	var _iframe = $('<div  class="mhyDialog"><iframe width="100%" height="100%" frameborder=0 openmodel="1" id="222"  ></iframe></div>');

		 $(rootWin.document.body).append(_iframe);
		_iframe.dialog(params);
		_iframe.dialog('open');
	    _iframe.find("iframe").attr("src", "${ctx}/business/personagency/params/paramsPayment/add.xf");
    }
        </script>
	...
	<body class="navbar-fixed breadcrumbs-fixed overflowyh">
			...
			<div class="navbar-container" id="navbar-container">
				...
				<div class="navbar-header pull-right" role="navigation">
					...
					<ul class="nav ace-nav navClick">
						...
						<li class="changeSkin" tabindex="0" onblur="blurFunction();" onclick="showSkin();">
		...
		<div class="main-container h100p" id="main-container">
			...
			<div class="main-container-inner h100p">
				<div class="sidebar sidebar-fixed h100p" id="sidebar">
										    ...
										    <c:forEach items="${subsysResource.subSysResourceList}" var="sub_subsysResource">
										        <li>
													<a href="#" class="dropdown-toggle" url="${sub_subsysResource.funmapping}" txt="${sub_subsysResource.funname}"  onclick="_onClick(this)">
														...
														<span style="position: absolute;left: 0; top: 5px;">......</span>
														<!-- <b class="arrow icon-angle-right"></b> -->
													</a>
												</li>
										    </c:forEach>
		...
		<script type="text/javascript">
			var navh = $("#navLeft").height();
			var winh = window.innerHeight;
			// 菜单栏滚动条实现
			function navLeftScroll(){
				var navSubHeight = winh -75 - navh ;
				//alert("navleft...."+navh+"winh"+winh);
				if($("#nav-li-wrap").children(".submenu")){
					$(".define-height").css("max-height",navSubHeight);
				}

			}
			navLeftScroll();
			$(window).resize(function(){
				$(".define-height").css("max-height",0);
				var winh = window.innerHeight;
				var navh = $("#navLeft").height();
				var navSubHeight = winh -75 - navh ;
				if($("#nav-li-wrap").children(".submenu")){
			...
			// 菜单栏滚动条2
			// function navLeftBar(){
			// 	winh = window.innerHeight-320;
			// 	navh = $("#navLeft").height();
			// 	navh2 = window.innerHeight-60;
			// 	$("#navLeft").height(navh2);
			// 	Ps.initialize(document.getElementById('navLeft'));
			// }
			// navLeftBar();
			// $(window).resize(function(){
			// 	navLeftBar();
       ...
       // ifid();
       		...
       		//主题色切换显示隐藏
       		function showSkin(){
       			$(".changeSkin ul.dropdown-menu").toggle();
       		}

       		function hideSkin(){
       			$(".changeSkin ul.dropdown-menu").hide();
       		}

       		function blurFunction(){
       			var timeId = setTimeout("hideSkin()",100);
       			clearTimeout(timeId);
       		}

       		//左侧菜单第一项默认展开
       		$("#navLeft").children("li").eq(0).addClass("open").children(".submenu").eq(0).css("display","block");
		</script>
