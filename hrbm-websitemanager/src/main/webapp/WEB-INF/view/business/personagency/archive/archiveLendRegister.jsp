				 ...
				 if(!data){
					 ...
					 if(data.ARCHIVESTATUS==2){
						 ...
						 if(data.ARCHIVEFLOWSTATUS==1){
							 bootbox.alert("当前档案正在借阅登记！不能调出登记！");
						 }else if(data.ARCHIVEFLOWSTATUS==2){
							 bootbox.alert("当前档案正在借阅确认！不能调出登记！");
						 }else if(data.ARCHIVEFLOWSTATUS==3) {
							 bootbox.alert("当前档案正在返还登记！不能调出登记！");
						 }else if(data.ARCHIVEFLOWSTATUS==6){
							 bootbox.alert("当前档案正在调出确认！档案已经调出，不能调出登记！");
						 }else{
							 if(data.ARCHIVERECEIVESTATUS==1){
								 bootbox.alert("当前档案没有入库！不能调出登记！");
							 }else{
								 $("#archiveLendRegisterResultList").load("${ctx}/business/personagency/archive/archiveLend/resultList.xf?personId="+ data.PERSONID);
							 }
						 }
					 }else{
						//在档
						 if(data.ARCHIVEFLOWSTATUS==1){
							 ...
							 if(data.ARCHIVERECEIVESTATUS==1){
								 bootbox.alert("当前档案在档案，但是没有入库！不能调出登记！");
							 }else{
								 $("#archiveLendRegisterResultList").load("${ctx}/business/personagency/archive/archiveLend/resultList.xf?personId="+ data.PERSONID);
							 }
						 }
...
<body>
	<div class="container-fluid">
		...
		<div id="archiveLendRegisterResultList"></div>
	</div>
</body>
</html>