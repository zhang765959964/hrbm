...
<script type="text/javascript">
	$(function() {
			 ...
			 callBack:function(data){
				 if(!data){
					 ...
					 if(data.ARCHIVESTATUS==2){
						 //调出
						 bootbox.alert("当前档案已经调出！不能办理工资标准调整！");
					 }else if(data.ARCHIVESTATUS==3){
						 //死亡
						 bootbox.alert("当前档案已经死亡！不能办理工资标准调整！");
					 }else{
						//在档
						 if(data.ARCHIVEFLOWSTATUS==1){
							 bootbox.alert("当前档案正在借阅登记！不能办理工资标准调整！");
						 }else if(data.ARCHIVEFLOWSTATUS==2){
							 bootbox.alert("当前档案正在借阅确认！不能办理工资标准调整！");
						 }else if(data.ARCHIVEFLOWSTATUS==3) {
							 bootbox.alert("当前档案正在返还登记！不能办理工资标准调整！");
						 }else if(data.ARCHIVEFLOWSTATUS==5){
							 bootbox.alert("当前档案正在调出登记！不能办理工资标准调整！");
						 }else{
							 if(data.ARCHIVERECEIVESTATUS==1){
								 bootbox.alert("当前档案在档案，但是没有入库！不能办理工资标准调整！");
							 }else{
								 $("#wageStandardAndList").load("${ctx}/business/personagency/storage/wageStandard/resultList.xf?personId="+ data.PERSONID);
							 }
						 }
					 }
				 }
		...
		$("#wageStandardAndList").load("${ctx}/business/personagency/storage/wageStandard/resultList.xf");
	})
</script>
</head>
<body>
	<div class="container-fluid">
		...
		<div id="wageStandardAndList"></div>
	</div>
</body>
</html>