...
$(".mhyInputReset").click(function(){
	...
	function initTable() {
		...
		var columns= [[{field:'',title:'Id号',checkbox:'true',width:40,align:'center'},
	               ...
	               /*     {field:'pxSignUpId',title:'操作',width:80,formatter: function(value,row,index){	 
				           return "<a href=\"javascript:void(0)\" onclick=\"editTrainCourse('"+value+"')\" >编辑</a>";
							}
					 } */
      	        	]];
		$('#trainSignUpList').bootstrapTable({
			url : '${ctx}/business/dissupport/train/trainSignUp/listJson.xf?courseId=${courseId}',
			columns: columns,
		    method:"post",
    ...
    */
	...
	function exportTrainSignUp(obj){
		...
		var pxSignUpId="";
		if(flag==1){
			var row = $('#trainSignUpList').bootstrapTable('getAllSelections');
			if(!row||row.length<=0){
		...
		window.location.href="${ctx}/business/dissupport/train/trainSignUp/export.xf?pxSignUpId="+pxSignUpId+"&courseId=${courseId}";
	}
	/**
	 * 查看标记
	 */
	function checkTrainSignUp(){
		...
		bootbox.confirm({ 
		    ...
		    callback: function(r){ 	    	
		    	 if (r){   
		 	    	$.ajax({ url: '${ctx}/business/dissupport/train/trainSignUp/updateBatch.xf',
	...
	function refreshTable(){
		if(leftClickTreeNode){
			...
			$('#trainSignUpList').bootstrapTable('refresh', {query:{
				colId: id
			  }				
			});
		}else{
	...
	function refresh(){
		$("#trainSignUpList").bootstrapTable('refresh');
	}
	
	function doSearch(){
			var params=$("#searchForm").serialize();
		
			$("#trainSignUpList").bootstrapTable('refresh', {url:"${ctx}/business/dissupport/train/trainSignUp/listJson.xf?courseId=${courseId}&"+params});
		}
						...
						<div class="keep-open btn-group mhyBtnLeft" title="Columns">
							...
							<div class="col-xs-12">
								<div class="table-responsive">
									<table id="trainSignUpList"  class="datagridClass"  >
									</table>
								</div>
							</div>
						</div>
