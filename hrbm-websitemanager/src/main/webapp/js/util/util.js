...
function dateBetween(beginDate,endDate){
	...
	if(!reg.test(beginDate)){
		...
		return flag;
	}else if(!reg.test(beginDate)&&reg.test(endDate) ){
		return flag;
	}else if(!reg.test(endDate)&&reg.test(beginDate)){
		return flag;
	}else{
		var end  = endDate.substring(0,10).split('-');
		var begin = beginDate.substring(0,10).split('-');
		var end1 = new Date(end[0],end[1],end[2]);	//结束时间
		var begin1 = new Date(begin[0],begin[1],begin[2]);	//开始时间
		if(Date.parse(begin1)-Date.parse(end1)>0){
			flag =true;
		}else{
			flag = false;
		}
...
//蒋鑫  验证开始日期和结束日期，开始和结束日期都可以为空
function jxDateBetween(beginDate,endDate){
	// true 验证不通过，false 验证通过
	var flag = true;
