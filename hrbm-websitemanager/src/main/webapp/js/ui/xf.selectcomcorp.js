...
(function($) {
	...
	function searchData(target,personData){
		...
		var opts = $.data(target, 'selectcomcorp').options;
		$.ajax({
			type:"post",
			url:CTX+"/sys/component/uums/selComCorp.xf",
	...
	$.fn.selectcomcorp = function(options, param) {
		if (typeof options == 'string') {
			return $.fn.dataexp.methods[options](this, param);
		}
		options = options || {};
		return this.each(function() {
			var state = $.data(this, 'selectcomcorp');
			if (state) {
				$.extend(state.options, options);
			} else {
				state = $.data(this, 'selectcomcorp', {
					options : $.extend({}, $.fn.selectcomcorp.defaults, options)
				});
				init(this);
			}
	...
	$.fn.selectcomcorp.methods = {
		options : function(jq) {
			return $.data(jq[0], 'menu').options;
		}
	};
	$.fn.selectcomcorp.defaults = {
		autoOpen : false,
		width : 865,
		height:950
	};
