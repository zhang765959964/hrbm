...
$(function(){
		var columns= [
		              	   {name:'ID',lable:'',hidden:true,key:true,frozen : true},
		                   ...
		                   {name:'ARCHIVESTATUSNAME',label:'档案状态',align:'center',frozen : true},
		                   {name:'ARCHIVEFLOWSTATUSNAME',label:'借调状态',align:'center'},
	...
	function initDataExp(){
		$('#exp').dataexp({
			...
			menuItem:[{title:"导出全部数据",paramCallBack:function(){
							var ids=$("#archivesList").jqGrid("getDataIDs");
							if(ids&&ids.length>0){
								dataJson.menuNumber='1';
								return dataJson;
							}else{
				        		...
				        		var ids=$("#archivesList").jqGrid("getDataIDs");
				      		    if(ids&&ids.length>0){
						      			...
						      			$.each(ids,function(i,e){
						      				var rowData=$("#archivesList").jqGrid("getRowData",e);
