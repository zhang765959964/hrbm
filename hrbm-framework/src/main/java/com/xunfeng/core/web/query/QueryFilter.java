package com.xunfeng.core.web.query;
...
import com.xunfeng.core.page.PageUtils;
import com.xunfeng.core.util.ColumnNameUtil;
import com.xunfeng.core.util.StringUtil;
import com.xunfeng.core.web.util.RequestUtil;
/**  
* @company: 河南讯丰信息技术有限公司 
* @Description: 将查询条件封装成Map对象 ，封装分页参数构建PageBean，封装排序条件
* @author wanghan 
* @createDate 2015年4月23日-上午10:38:12 
* @version V1.0   
*/
...
//		this.request=request;
		...
		try{
			...
			String orderField=RequestUtil.getString(request, "sort");
			String orderSeq =RequestUtil.getString(request, "order");
			...
			if(isLg){
				...
				Integer pageSize=RequestUtil.getInt(request, "rows",PageBean.DEFAULT_PAGE_SIZE);
	 ...
	 * 如果需要碰到需要分页的情况，	请{@link com.xunfeng.core.web.util.RequestUtil#getQueryMap(HttpServletRequest) 参考}定义的参数规格。
	 * 
...
//		this.request=request;
	...
	//带排序的
	public QueryFilter(HttpServletRequest request,Class clazz){
...
//		this.request=request;
		try{
			...
			String orderSeq=request.getParameter("order");
			if(null!=orderField){
				String[] orderFields =  orderField.split(",");
				String columnName="";
				for(String o:orderFields){
					columnName += ColumnNameUtil.getColumnNameThroughAnnotaion(clazz, o)+",";
				}
				map.put("orderField", columnName.substring(0, columnName.length()-1));
				map.put("orderSeq", orderSeq);
			}
			filters=map;
		 }catch(Exception ex){
