package com.xunfeng.core.excel.reader;
...
public class ExcelReader {
    ...
    private boolean isBlankRow(List<FieldEntity> fieldEntityList){
    	
    	boolean flag=false;
    	for(FieldEntity fieldEntuty:fieldEntityList){
    		String value=fieldEntuty.getValue();
	...
	private List<FieldEntity> getLine(HSSFSheet sheet, int row) {
		...
		// 遍历所有列
		int columnsLength=this.excelReaderConfig.getColumns().length;
		for (int i = colStart; i < filledColumns; i++) {
			// 取得当前单元格
			cell = rowline.getCell(i);
			if(i-colStart>columnsLength-1){
				break;
			}
...
//		}
	...
	public static void main(String[] args) throws Exception {
		...
		File file = new File("d:\\test.xls"); // 把testEntity.xls文件复制到d:
